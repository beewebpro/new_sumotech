                            ...
                            @forelse($upcomingJobs as $job)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-900">
                                        ...
                                        <span
                                            class="text-gray-500 text-xs">({{ $job['next_run']->diffForHumans() }})</span>
                                    </td>
                                    <td class="px-6 py-4 text-sm space-x-1">
                                        @if ($job['without_overlapping'])
                                            <span class="px-2 py-0.5 text-xs bg-blue-100 text-blue-700 rounded-full">No
                                                Overlap</span>
                                        @endif
                                        @if ($job['run_in_background'])
                                            <span
                                                class="px-2 py-0.5 text-xs bg-purple-100 text-purple-700 rounded-full">Background</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-8 text-center text-gray-500 italic">No scheduled jobs
                                        configured</td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <form method="GET" action="{{ route('automation-reports.index') }}"
                    ...
                    <div>
                        ...
                        <select name="command_name" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                            ...
                            @foreach ($commandNames as $name)
                                <option value="{{ $name }}"
                                    {{ request('command_name') == $name ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="trigger" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                            <option value="">All</option>
                            <option value="schedule" {{ request('trigger') == 'schedule' ? 'selected' : '' }}>Schedule
                            </option>
                    ...
                    <div class="flex gap-2">
                        <button type="submit"
                            class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm transition">Filter</button>
                        <a href="{{ route('automation-reports.index') }}"
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Finished</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Command</th>
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($log->status === 'success')
                                            <span
                                                class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Success</span>
                                        @elseif($log->status === 'failed')
                                            <span
                                                class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Failed</span>
                                        @elseif($log->status === 'running')
                                            <span
                                                class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Running</span>
                                        @else
                                            <span
                                                class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">{{ ucfirst($log->status) }}</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                        @if ($log->duration_seconds)
                                            @if ($log->duration_seconds >= 60)
                                                {{ floor($log->duration_seconds / 60) }}m
                                                {{ (int) ($log->duration_seconds % 60) }}s
                                            @else
                                                {{ number_format($log->duration_seconds, 1) }}s
                                            @endif
                                        @elseif($log->status === 'running' && !empty($log->running_duration_seconds))
                                            @if ($log->running_duration_seconds >= 60)
                                                {{ floor($log->running_duration_seconds / 60) }}m
                                                {{ (int) ($log->running_duration_seconds % 60) }}s (running)
                                            @else
                                                {{ number_format($log->running_duration_seconds, 1) }}s (running)
                                            @endif
                                        @else
                                            -
                                        @endif
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <span
                                            class="px-2 py-0.5 text-xs rounded-full {{ $log->trigger === 'schedule' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700' }}">
                                            {{ ucfirst($log->trigger) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-600 max-w-xs">
                                        @if ($log->output)
                                            <div class="text-xs text-gray-700 truncate" title="{{ $log->output }}">
                                                {{ Str::limit($log->output, 80) }}</div>
                                        @endif
                                        @if ($log->meta_data)
                                            <div class="flex flex-wrap gap-1 mt-1">
                                                @foreach ($log->meta_data as $key => $value)
                                                    @if (!is_array($value))
                                                        <span
                                                            class="text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded">{{ $key }}:
                                                            {{ $value }}</span>
                                                    @else
                                                        @php
                                                            $displayValues = $value;
                                                            $isSmallList = count($displayValues) <= 5;
                                                            $displayText = $isSmallList
                                                                ? implode(', ', $displayValues)
                                                                : count($displayValues) . ' items';
                                                        @endphp
                                                        <span
                                                            class="text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded">{{ $key }}:
                                                            {{ $displayText }}</span>
                                        ...
                                        @if ($log->error_message)
                                            <div class="text-xs text-red-600 mt-1 truncate max-w-xs"
