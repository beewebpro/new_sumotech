...
class GenerateChapterTtsBatchJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach ($chapters as $chapter) {
            ...
            $initData = method_exists($initResponse, 'getData') ? $initResponse->getData(true) : [];
            $chunkCount = (int) ($initData['total_chunks'] ?? $chapter->chunks()->count());
            $totalSteps += $chunkCount + 1; // +1 for merge
        }
        ...
        foreach ($chapters as $chapter) {
            ...
            foreach ($chunks as $chunk) {
                $this->updateProgress([
                    ...
                    'chunk_percent' => $totalChunks > 0 ? (int) floor(($chunk->chunk_number / $totalChunks) * 100) : 0,
                ]);
